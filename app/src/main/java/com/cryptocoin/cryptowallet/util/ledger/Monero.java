/*
 * A quick and hacky Java implementation of most of
 * https://github.com/LedgerHQ/ledger-app-monero/blob/master/tools/python/src/ledger/monero/seedconv.py
 *
...
public class Monero {
    ...
    private byte[] mkey;
    private byte[] mchain;
    private byte[] monero_ki;
    private byte[] monero_ci;
    private byte[] view_key;
    private byte[] spend_key;

    private static byte[] NKFDbytes(String str) {
        return Normalizer.normalize(str, Normalizer.Form.NFKD).getBytes();
    }

    private static char[] NKFDchars(String str) {
        return Normalizer.normalize(str, Normalizer.Form.NFKD).toCharArray();
    }

    private static byte[] fixByteArray32(byte[] b) {
        if ((b.length > 33)) throw new IllegalStateException();
        if ((b.length == 33) && (b[0] != 0)) throw new IllegalStateException();
        if (b.length == 33)
            return Arrays.copyOfRange(b, 1, 33);
        else
            return b;
    }

    private static byte[] intToBytes(int i, int bytes) {
    ...
    private void makeSeed(String mnemonic, String passphrase)
        ...
        seed = skf.generateSecret(spec).getEncoded();
    }
    ...
    private String getWords() {
        ...
        for (int i = 0; i < spend_key.length / 4; i++) {
            ...
            long w1 = val % wordList.length;
            long w2 = ((val / wordList.length) + w1) % wordList.length;
            long w3 = (((val / wordList.length) / wordList.length) + w2) % wordList.length;

            words.add(wordList[(int) w1]);
            words.add(wordList[(int) w2]);
            words.add(wordList[(int) w3]);
        }

        StringBuilder mnemonic = new StringBuilder();
        StringBuilder trimmedWords = new StringBuilder();

        for (String word : words) {
            mnemonic.append(word).append(" ");
            trimmedWords.append(word.substring(0, ENGLISH_PREFIX_LENGTH));
        }
        CRC32 crc32 = new CRC32();
        crc32.update(trimmedWords.toString().getBytes(StandardCharsets.UTF_8));
    ...
    public static final String[] ENGLISH_WORDS = {
            "abbey",
            "abducts",
            "ability",
            "ablaze",
