...
class ExchangeRateImpl implements ExchangeRate {
    ...
    ExchangeRateImpl(final JSONObject jsonObject, final boolean cryptocoinAssets) throws JSONException, ExchangeException {
        try {
            ...
            baseCurrency = cryptocoinAssets ? quoteC : baseC;
            quoteCurrency = cryptocoinAssets ? baseC : quoteC;
            JSONObject quote = quotes.getJSONObject(key);
            double price = quote.getDouble("price");
            this.rate = cryptocoinAssets ? (1d / price) : price;
